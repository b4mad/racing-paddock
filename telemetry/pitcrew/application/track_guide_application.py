...
class Turn:
    ...
    def set_next_response(self, distance=0):
    ...
    def reset(self, distance):
        """find the first note that will be played
        at this distance
        """
        ...
        # get all responses of the first batch
        resp = []
        sort_key = self._current_response.sort_key
        while self._current_response.sort_key == sort_key:
            resp.append(self._current_response)
            self.set_next_response()

        # sort by diff to distance
        resp.sort(key=lambda x: abs(distance - x.at))
        for response in resp:
            if response.at > distance:
                self.set_response(response)
                break

    def set_response(self, response):
        self.sort_responses()
        while self._current_response != response:
            self.set_next_response()
...
class TrackGuideApplication(Application):
    ...
    def init_recon_notes(self, distance):
        distance = self.distance_add(distance, self.RECON_ADD_DISTANCE)
    ...
    def tick(self):
        ...
        if self.changed_coaching_style():
            if self.is_recon_laps():
                ...
                self.init_recon_notes(self.distance)
            else:
    ...
    def on_reset_to_pits(self, distance: int, telemetry: dict, now: datetime.datetime):
        ...
        if self.is_recon_laps():
            self.init_recon_notes(distance)
    ...
    #         if finish_at:
    #             response = self.build_response(track_note.message,
    # finish_at=at, max_distance_delta=max_distance_delta)
    #         else:
